...
def take_rotate_image(C, path, fastener, rotate):
...
def run_sim(model_path, image_path, model_name, copies_to_take):
    for copy in range(copies_to_take):
        ...
        rotate = bpy.data.objects['Empty']
        dir_name = f"{model_name}_{copy}" 
        dir_name += f"/{dir_name}"
        final_image_path = os.path.join(image_path, dir_name)
        take_rotate_image(C, final_image_path, fastener, rotate)
...
for i in range(4):
    ...
    C.scene.render.filepath = f"/Users/evliu/evan/eng/ENPH459/images/ewa_og_{i}.jpg"
...
base_path_to_model = "/Users/evliu/evan/eng/ENPH459/cads/"
...
rotate_and_image(fastener, rotate)
